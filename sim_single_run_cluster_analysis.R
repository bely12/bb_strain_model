...
pop_data <- read.delim('cr_len20_vec1000_yrs250_variant_frequencies.tsv', header = T, colClasses = c('character','numeric','numeric','character')) # upload
...
### pca analysis ###
pca_result <- prcomp(binary_matrix_numeric, center = TRUE, scale. = TRUE)
...
ggplot(pca_data, aes(x = PC1, y = PC2, color = cluster)) +
  ...
  labs(title = "PCA with Hierarchical Clustering",
       subtitle = 'Immune selection, antigen len=20, vector pop=1000, years=250, mut=0.01',
...
InGroupDistance(run_pop) # in cluster distance
OutGroupDistance(run_pop) # cross cluster distance
